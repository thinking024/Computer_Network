...
int ProxyThread(ProxyParam* lpParameter);
...
int ProxyThread(ProxyParam* lpParameter) {//socket响应线程
	char Buffer[MAXSIZE];
	char* CacheBuffer;
...
bool ConnectToServer(SOCKET* serverSocket, char* host) {//代理访问server
	...
	printf("****HOST:%s\n", host);
	if (strcmp("computer.hnust.edu.cn/", host) == 0 || strcmp("computer.hnust.edu.cn", host) == 0)
	{
		printf("BLOCK http://computer.hnust.edu.cn/\n");
		return false;
	}
